#pragma once

#ifdef KL2EDK_INCLUDE_MESSAGES
  #pragma message ( "Including 'Quat_i.h'" )
#endif

////////////////////////////////////////////////////////////////
// THIS FILE IS AUTOMATICALLY GENERATED -- DO NOT MODIFY!!
////////////////////////////////////////////////////////////////
// Generated by kl2edk version 2.7.0
////////////////////////////////////////////////////////////////

#include <FabricEDK.h>
#if FABRIC_EDK_VERSION_MAJ != 2 || FABRIC_EDK_VERSION_MIN != 7
# error "This file needs to be rebuilt for the current EDK version!"
#endif

// Core\Clients\CAPI\KL2EDK.cpp:812
#include "global.h"
// Core\Clients\CAPI\KL2EDK.cpp:864
#include "Vec3_i.h"

namespace Fabric {
namespace EDK {
namespace KL {


// KL struct 'Quat_i'
// Defined at Mat33_i.kl:52:1

struct Quat_i
{
  typedef Quat_i &Result;
  typedef Quat_i const &INParam;
  typedef Quat_i &IOParam;
  typedef Quat_i &OUTParam;
  
  ::Fabric::EDK::KL::Vec3_i v;
  ::Fabric::EDK::KL::SInt32 w;
};

inline void Traits<Quat_i>::ConstructEmpty( Quat_i &val )
{
  Traits< ::Fabric::EDK::KL::Vec3_i >::ConstructEmpty( val.v );
  Traits< ::Fabric::EDK::KL::SInt32 >::ConstructEmpty( val.w );
}
inline void Traits<Quat_i>::ConstructCopy( Quat_i &lhs, Quat_i const &rhs )
{
  Traits< ::Fabric::EDK::KL::Vec3_i >::ConstructCopy( lhs.v, rhs.v );
  Traits< ::Fabric::EDK::KL::SInt32 >::ConstructCopy( lhs.w, rhs.w );
}
inline void Traits<Quat_i>::AssignCopy( Quat_i &lhs, Quat_i const &rhs )
{
  Traits< ::Fabric::EDK::KL::Vec3_i >::AssignCopy( lhs.v, rhs.v );
  Traits< ::Fabric::EDK::KL::SInt32 >::AssignCopy( lhs.w, rhs.w );
}
inline void Traits<Quat_i>::Destruct( Quat_i &val )
{
  Traits< ::Fabric::EDK::KL::SInt32 >::Destruct( val.w );
  Traits< ::Fabric::EDK::KL::Vec3_i >::Destruct( val.v );
}
} // namespace KL
} // namespace EDK
} // namespace Fabric


